...
- **Cache dominance (key normalization caveat):** median `cached_input/input` = **88.8%** (`cached/total` **87.7%** — the two denominators differ; codex's own token schema, not directly comparable to OMP's `cacheRead/total` 96–98%). **SUPERSEDED-FIGURE NOTE (erratum #1, `historical-hypotheses.md`):** the earlier "productive (output+reasoning) median 12,070 tok = 1.26%, ~50× overstate" double-counted reasoning (`reasoning_output_tokens` ⊆ `output_tokens`, never added) and omitted non-cached input. Corrected — model-generated output median **8,567 tok = 0.93%**; productive = output + non-cached input, median **124,297 tok = 12.28%**; raw `total_tokens` overstates codex cost **~5.6×** (not ~50×). Cross-harness cost comparisons use the productive figure, never raw `total_tokens`. The `cx-cost-permodel-*` items (and the `out+reas` table column below) still record the superseded `output+reasoning` value per model — read them through this erratum.
