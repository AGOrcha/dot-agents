...
- **D2 — two tiers: fast `make gate` (every push) + `make gate-cross` (pre-merge).**
  - **`make gate`** (every push, deterministic, seconds-to-low-minutes): build + vet (incl. `GOOS=windows` cross-compile), **full per-file coverage enforce — exactly as CI runs it** (`COVERAGE_FILE_MODE=enforce` over the full-suite single-OS profile, using CI's same `scripts/coverage-exceptions.txt` allowlist + 95% threshold), native sonar (D3). *(Revised 2026-06-25 — a cross-harness (Codex) adversarial review found the earlier "diff-scoped to changed files" design reopened pass-local-fail-CI holes: a changed platform-specific file has zero coverage rows on a single-OS run so diff-scoping passed it locally; and a test-only change that drops a production file <95% is skipped entirely because no production `.go` "changed". Full enforce closes both — it is the same speed (the full suite already runs to produce the profile) and faithful to CI by construction.* The **one residual gap** — a genuinely NEW platform-only file (e.g. a brand-new `*_windows.go`) that has zero coverage rows on the single-OS local run — is **`gate-cross`'s job** (its merged multi-OS profile sees it); the fast tier just notes it.)
  - **`make gate-cross`** (pre-merge, heavier): real **Windows tests via `ssh pap-h@pap-home.local`** for changed packages (closes the runtime gap cross-compile misses), and a **merged local+Windows coverage profile** → true multi-OS per-file enforce == CI. Driven by the orchestrator (or CI) before merge, not on every push. Also closes the NEW-platform-only-file gap noted above.
...
- **D4 — CI per-file coverage is the contract; the local gate matches it, never weaker.** The local gate must be **≥** CI in strictness (R1 fixed): **full per-file enforce locally, matching CI's enforce mode + the same `coverage-exceptions.txt` allowlist + threshold** (not diff-scoped — see the D2 revision); full merged multi-OS per-file in `gate-cross`/CI. The only file the single-OS local gate cannot see is a NEW platform-only file with zero local rows — that is `gate-cross`'s job. Until t5 wires CI to literally call `make gate`, the hook and CI invoke the **same `scripts/coverage-gate.sh` in enforce mode with the same allowlist + threshold**, so there is zero coverage-mode drift even though the call site is duplicated.
...
1. `make gate` exits non-zero iff a merge-blocking CI gate would fail; deterministic across repeated runs on an unchanged tree (no flake).
...
4. `make gate` enforces **per-file** ≥95% over the **full** profile (matching CI's enforce mode + shared `coverage-exceptions.txt` allowlist), not per-package and not diff-scoped.
