# Scoped Knowledge Graphs — Product Contract

**Status:** draft (canonical)
**Written:** 2026-04-21
**Revised:** 2026-04-21 (post-Codex-adversarial-review)
**Supersedes:** spec.1.md, spec.2.md (both retained as historical snapshots)
...
**Relative to v1 (spec.1.md).** v1 treated staleness as a *freshness policy*
with TTL, source-hash, and ETag as interchangeable candidates. That framing
was wrong. Domain knowledge does not decay on a clock — a fact stays true
until something drives a change. This revision rebuilds §2.5 and §5 around
**driver events**: an entry is fresh by default and becomes stale only when
an identifiable event invalidates it. Time-based signals are demoted from
"staleness" to **review-nudges** (§2.7) — a separate dimension from truth.

**Relative to the first event-driven rewrite (spec.2.md).** An adversarial
review caught two contract-level defects:

1. **Legacy config semantics were ambiguous.** The v2 rewrite said legacy
   `agentsrc.kg.graph_home` / `agentsrc.kg.backend` configs still work as a
   single-scope `repo` chain, but did not commit to what staleness behavior
   the translation carries. Combined with §5.3's "a scope with no drivers
   produces no staleness signals," an untouched legacy config would appear
   compatible while silently providing no invalidation — a silent semantic
   loss. §3.1 now makes the translation explicit and requires a one-time
   runtime diagnostic so the absence is loud, not silent.

2. **Cross-scope contradictions were modeled inconsistently.** The v2 rewrite
   named `contradiction` as a stale reason *and* required contradictions to
   surface across scopes at read time, while also committing to write-time-
   only propagation. The two requirements cannot coexist without either a
   separate materialization job (breaking §5.6) or dropping `fired_at` for
   cross-scope collisions (breaking §5.4). §2.5 driver 4 and §3.2/§3.5/§5.4
   now split the behavior: same-scope contradictions fire a write-time driver
   and produce `stale: { reason: "contradiction" }`; cross-scope disagreements
   are read-time metadata in the `contradictions` field only, and do **not**
   emit a stale tag. Both scopes' facts remain fresh; precedence selects the
   returned answer.

Everything else — scope chain, per-scope backends, reads-fan-out / writes-
target, provenance, propagation along derivation edges, resolver purity —
is unchanged from the first event-driven rewrite.
...
3. **Explicit revocation.** A writer marks an entry revoked. Revocations
   ...
   appealed.
4. **Contradiction arrival (same-scope only).** A new write asserts the
   opposite of an existing entry with the same node id *in the same scope*.
   Detected at write time; fires a driver on the older entry so readers see
   `stale: { reason: "contradiction" }`. **Cross-scope disagreements are not
   a staleness driver** — they are surfaced as read-time metadata in the
   `contradictions` field (§3.2). Both scopes' facts remain fresh; precedence
   selects the returned answer. See §5.12 for the full commitment.
...
declare a driver gets the wrong behavior by default. This revision takes
the opposite default: **no driver declared → no staleness signal at all.**
The entry is assumed fresh, and **review-nudges (§2.7)** — a separate
dimension — handle the case where humans want periodic confirmation.
...
- **Legacy config translation.** An `agentsrc.kg` block without `scopes`
  — using only the legacy `graph_home` / `backend` fields — is translated
  at load time to a single-scope `repo` chain with **no drivers active
  and no review-nudge**. This preserves today's no-staleness behavior
  *exactly*: a legacy config produces no stale tags and no review-due
  markers. **Opting into the event-driven model is explicit**: the user
  must migrate to `kg.scopes` and declare `drivers:` per scope.
- **Legacy diagnostic is mandatory.** When the runtime detects a legacy
  config (no `scopes` list), it emits a one-time diagnostic at session
  start and in `kg scopes status` output, naming the missing staleness
  surface and pointing at the migration path. A legacy config must never
  look indistinguishable from a scoped config that chose to declare no
  drivers — the diagnostic makes the absence loud.
...
- Cross-scope disagreements (same node id, different claims across scopes)
  surface in a `contradictions` metadata field even when precedence already
  picked a winner. **These are fresh-vs-fresh disagreements**: neither side
  gets a `stale` tag. The `contradictions` field is read-time-derived
  metadata; it does not mutate stored state.
...
- **Same-scope contradiction at write.** If a write lands on an existing
  node id in the same scope with an opposing claim, the runtime fires a
  `contradiction` driver on the *older* entry (tags it
  `stale: { reason: "contradiction", because: [<new-entry-id>] }`) and
  surfaces a write-time warning the writer can override.
...
- `kg scopes status` surfaces the legacy-diagnostic state when the
  config is legacy (§3.1).
...
- An entry invalidated by a driver is returned with
  ...
  **`reason: "contradiction"` covers same-scope write-time contradictions
  only.** Cross-scope disagreements are reported via the `contradictions`
  metadata field (§3.2) and do *not* emit a `stale` tag.
...
> **O5 resolution (2026-06-23, single-source).** The staleness-driver and
> derivation-propagation sub-decisions in §4.1 and §4.2 are **resolved** by
> O5 of the
> [graph-backend-adapter-contract open-questions dossier](../graph-backend-adapter-contract/open-questions-resolutions.md#o5--scoped-kg-driver-taxonomy--derivation-propagation-depthdecay):
> all five spec recommendations are adopted as written —
> **source mutation = content-hash change**, **revocation = new note with
> `revokes: <id>`**, **code-graph derivation depth = 1 hop** with
> **unbounded note→note chains** (opt-in per the adapter contract §7.3
> `derivation: true` gate), **taint persists until an explicit
> `kg refresh --scope`** (never fade-on-read), and the **edge-type allowlist
> = load-bearing `LinkKind`s (`documents`, `implements`, `decided_on`),
> excluding `mentions`**. O5 shows each is *forced* by an already-ratified §5
> design commitment, not a free choice. The §4.1/§4.2 text below is retained as
> the rationale-of-record; the canonical answers are O5. The remaining §4.3–§4.9
> sub-decisions are unaffected and stay open.

### 4.1 Driver taxonomy — what counts, what fires

Recommendations below are **resolved per O5** (content-hash mutation;
new-note revocation). The plan must pin down:
...
- **Contradiction.** Same-scope detection at write time fires a driver on
  the older entry (§3.3). Cross-scope detection happens at read time and
  produces only `contradictions` metadata — **never** a `stale` tag (§3.2,
  §5.12). The plan must not blur this boundary.
...
- **Depth limit.** **Resolved per O5: 1** for code-graph hops; **unbounded
  ...
  `derivation: true` opt-in.
- **Edge-type allowlist.** **Resolved per O5:** load-bearing `LinkKind`s
  (`documents`, `implements`, `decided_on`) propagate; weak kinds
  (`mentions`) do not.
- **Taint decay.** **Resolved per O5: persist until explicit
  `kg refresh --scope`** (never fade-on-read — fade-on-read would violate
  §5.6 write-time-only propagation and §2.8 resolver purity).
...
### 4.9 Legacy-diagnostic surface — cadence and silencing

§3.1 requires a one-time runtime diagnostic when legacy config is
detected. The plan must commit:

- **Cadence.** One-time-per-process vs. on every `kg` invocation vs.
  once until `.agentsrc.json` is touched. Recommended: once per process
  *and* always in `kg scopes status` output — the ambient message fades,
  the explicit status query always shows it.
- **Silencing.** A `kg.scopes.silence_legacy_diagnostic: true` flag for
  users who have read the message and intentionally want to stay on
  legacy config. Never silence by default.
...
staleness signals — entries are simply fresh. **See §3.1 for the legacy-
config diagnostic that keeps this absence loud rather than silent.**
...
tainted it. `reason: "contradiction"` covers same-scope write-time
contradictions only (§3.3); cross-scope disagreements do not produce
`stale` tags (§3.2, §5.12).
...
### 5.12 Cross-scope disagreement is metadata, not staleness

Two scopes that assert opposing claims at the same node id both remain
**fresh**. The disagreement surfaces via the `contradictions` metadata
field (§3.2); precedence selects which answer is returned. No `stale`
tag is emitted for this condition and no write event fires at read time.

**Why:** the write-time-only propagation commitment (§5.6) and the
resolver purity commitment (§2.8) together preclude stamping a `stale`
tag at read time. A cross-scope disagreement is not a contradiction in
the sense driver 4 is about — each scope's fact was written honestly in
its own authority; they just happen to disagree. Precedence resolves
which is *returned*; the `contradictions` field discloses that the
disagreement exists. Neither is staleness.

**Contrast with same-scope contradiction (driver 4).** A same-scope
write with a colliding id and opposing claim is a true contradiction:
only one of the two entries can be correct under the scope's own
authority. The newer write fires a driver on the older entry at write
time, so the stale tag has a legitimate write event to stamp.

### 5.13 Legacy-config diagnostic is mandatory

When the runtime detects a legacy `kg` block (no `scopes` list), it
emits a one-time diagnostic at session start and surfaces the state in
`kg scopes status`. The diagnostic names the missing staleness surface
and points at migration. A legacy config must never look
indistinguishable from a scoped config that chose to declare no
drivers — the diagnostic makes the absence loud.
...
- A legacy `.agentsrc.json` with only `kg.graph_home` still works
  without edits and produces the mandatory legacy diagnostic (§3.1,
  §5.13).
...
- `kg scopes status` reports per-scope backend, drivers, health,
  review-nudge policy, *and* legacy-diagnostic state.
...
- A **same-scope** write with a colliding id and opposing claim fires
  a driver on the older entry, which returns with
  `stale: { reason: "contradiction", because: [<new-id>], fired_at }`.
- A **cross-scope** node collision with opposing claims returns in
  `contradictions` metadata even after precedence picks a winner, and
  **neither side is tagged `stale`**.
...
- A cross-scope contradiction-materialization job (rejected in this
  revision as incompatible with §5.6; see §0 and §5.12).
