---
schema_version: 1
task_id: se9-commands-loud
parent_plan_id: swallowed-errors-loud-atomic
title: 'Slice 9 (commands): kg/lifecycle/workflow/toplevel should-be-LOUD'
summary: 'Verified the already-committed 21-file swallowed-errors slice 9 (kg/lifecycle/workflow/remove/refresh, commit d992cea8 on swallowed/9-commands-loud) against the per-file 95% coverage gate. Result: all 21 changed files pass — 9/11 changed source files are natively >=95%; the other 2 (commands/refresh.go 88.96%, commands/kg/query_lint_maintain.go 93.30%) carry pre-existing [legacy-tail] allowlist entries from before this slice touched them (cg6 ratchet-loop debt), and diffing against the pre-commit baseline profile confirmed every NEW error-surfacing branch this slice adds is 100% test-covered with zero regression (refresh.go actually improved 88.82%->88.96%, query_lint_maintain.go 93.10%->93.30%). No new tests or coverage-exceptions.txt entries were needed. go build/vet/test all green. Opened PR #365 (fix(commands): surface real errors vs legitimate absence...) off master, stop-before-merge as instructed. import.go/import_plugins.go intentionally excluded (separate disjoint sub-slice).'
files_changed:
...
verification_result:
    ...
    summary: ""
integration_notes: ""
created_at: "2026-07-09T06:10:45Z"
---

## Summary

Verified the already-committed 21-file swallowed-errors slice 9 (kg/lifecycle/workflow/remove/refresh, commit d992cea8 on swallowed/9-commands-loud) against the per-file 95% coverage gate. Result: all 21 changed files pass — 9/11 changed source files are natively >=95%; the other 2 (commands/refresh.go 88.96%, commands/kg/query_lint_maintain.go 93.30%) carry pre-existing [legacy-tail] allowlist entries from before this slice touched them (cg6 ratchet-loop debt), and diffing against the pre-commit baseline profile confirmed every NEW error-surfacing branch this slice adds is 100% test-covered with zero regression (refresh.go actually improved 88.82%->88.96%, query_lint_maintain.go 93.10%->93.30%). No new tests or coverage-exceptions.txt entries were needed. go build/vet/test all green. Opened PR #365 (fix(commands): surface real errors vs legitimate absence...) off master, stop-before-merge as instructed. import.go/import_plugins.go intentionally excluded (separate disjoint sub-slice).
