---
schema_version: 1
task_id: se8-registration-visibility
parent_plan_id: swallowed-errors-loud-atomic
title: 'Slice 8: agents/skills registration-step visibility'
summary: 'Implemented se8 (agents/skills registration-step visibility). appendAgentsRCStep (agents/new.go), appendSkillToAgentsRC + ensureUserSkillLinks (skills/new.go) now return their config.Load/LoadAgentsRC/rc.Save/os.Symlink errors instead of discarding them; CreateAgent/createSkill downgrade the terminal ui.SuccessBox to a ui.WarnBox naming exactly what failed to register, without rolling back the already-written AGENT.md/SKILL.md. os.IsNotExist on .agentsrc.json stays a legitimate best-effort skip (no warning) per the audit''s loud-vs-absent distinction. Exported wrapper signatures (CreateAgent, CreateSkill, AppendSkillToAgentsRC, SkillCreationNextSteps, EnsureUserSkillLinks) are unchanged — only the unexported IO-injected units gained the error return. Ripple: commands/agents/coverage_test.go and commands/skills/seams_test.go updated for the new two-return call sites (out of nominal write_scope but required to keep the packages compiling after the signature change; no other in-flight slice touches those two files). Added commands/skills/new_test.go (did not previously exist) with the registration-warning acceptance test + legitimate-absence test; commands/agents/new_test.go got the matching pair plus a local captureStdout helper. go build ./... and go vet ./... clean repo-wide; go test ./commands/agents/... ./commands/skills/... green. PR #359 opened base master <- swallowed/8-registration-visibility, stopped before merge, no AI trailer on commit/PR.'
files_changed:
...
verification_result:
    ...
    summary: ""
integration_notes: ""
created_at: "2026-07-09T03:11:11Z"
---

## Summary

Implemented se8 (agents/skills registration-step visibility). appendAgentsRCStep (agents/new.go), appendSkillToAgentsRC + ensureUserSkillLinks (skills/new.go) now return their config.Load/LoadAgentsRC/rc.Save/os.Symlink errors instead of discarding them; CreateAgent/createSkill downgrade the terminal ui.SuccessBox to a ui.WarnBox naming exactly what failed to register, without rolling back the already-written AGENT.md/SKILL.md. os.IsNotExist on .agentsrc.json stays a legitimate best-effort skip (no warning) per the audit's loud-vs-absent distinction. Exported wrapper signatures (CreateAgent, CreateSkill, AppendSkillToAgentsRC, SkillCreationNextSteps, EnsureUserSkillLinks) are unchanged — only the unexported IO-injected units gained the error return. Ripple: commands/agents/coverage_test.go and commands/skills/seams_test.go updated for the new two-return call sites (out of nominal write_scope but required to keep the packages compiling after the signature change; no other in-flight slice touches those two files). Added commands/skills/new_test.go (did not previously exist) with the registration-warning acceptance test + legitimate-absence test; commands/agents/new_test.go got the matching pair plus a local captureStdout helper. go build ./... and go vet ./... clean repo-wide; go test ./commands/agents/... ./commands/skills/... green. PR #359 opened base master <- swallowed/8-registration-visibility, stopped before merge, no AI trailer on commit/PR.
