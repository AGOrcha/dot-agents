...
- The four PR #40 loop-worker merge-backs (install, review, add, atomic-convergence). These reside in `.claude/worktrees/seam-di/.agents/active/delegation/` until the plan archives, then move to `history/loop-agent-pipeline/delegate-merge-back-archive/<date>/`.
- `~/.agents/profiles/loop-worker.md` — the global profile that names both paths (`active/delegation-bundles/` and `active/delegation/`) but does not bridge them.
- Implementation entry points: `commands/workflow/delegation.go` and `commands/workflow/delegation_fanout_test.go` — the fanout machinery where the contract-materialization extension would land.
