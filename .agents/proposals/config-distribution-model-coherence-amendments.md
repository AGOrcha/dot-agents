...
- **status:** **OWNER-RATIFIED (2026-06-27)** — **rev 2**, ready to fold into §15. All 8 forks (F1.1–F1.4, F2.1, F2.2, F3.1, F3.2) ratified; F1.3 (the last open one) resolved by owner ruling (see below). This proposal is now the canonical decision record the owner folds into the shipped `config-distribution-model` §15 spec (the fold itself is a separate gated step — §15 is not edited here).
...
- **F1.3 — default authority-rank ordering.** **[OWNER-RATIFIED 2026-06-27 — this was the LAST open fork; the owner refined the rev-2 recommended default (Option B, `org > team > user > repo`) by re-ranking the user to the BOTTOM of a single total authority order: `org > team > repo > user`.]** *Ratified ruling — TWO ORDERINGS (one authority order, one value order); the user is the LOWEST authority rung, NOT off-axis:*
  - **AUTHORITY ordering** — a **single total order**, **`org > team > repo > user`**, **deny-overrides**, **higher binds lower**. A higher scope's locks / value-locks / override-caps are absolute over every lower one. **repo MAY set its own local guardrails** (resource self-constraint — a repo can lock down *its own* surface) **but ranks BELOW team**, so a team lock still binds the repo.
  - **The USER is the LAST RUNG ON the authority chain (lowest authority), not off it.** The user IS on the authority order, at the **bottom**: it may emit locks / caps **only at its own scope**, and **every higher scope — including repo — overrides it**. This is what prevents a user from constraining a shared repo (repo out-ranks user) while keeping **one clean total order** rather than dropping the user off-axis. This is the refinement of Option B's `… > user > repo`: the user moves **below repo**, to the bottom rung — it is re-ranked, not removed.
  - **VALUE ordering** (choose values **within the locks the authority order leaves surviving**): **`user / repo / runtime`**, **most-local-wins** — the user still wins on **value** for its own preferences. This is the Phase-2 value-precedence merge, constrained by the authority pass.
  - **`product` = the floor**: **zero authority**, ships defaults, **everything overrides it**.
  - **`public` = VALUE-ONLY**: it may **supply values** but **any authority / lock claim it ships is IGNORED unless co-signed by a trusted root** — the supply-chain-signing safety rule (a foreign/public source can never bind via an unsigned lock).
  - **`runtime` and the project-local overlay = value-only, zero authority** (highest value-precedence, can set values but never locks).
  - *Grounding (why user-at-the-bottom-of-authority, below repo):* AWS SCP/IAM (an org SCP caps every account/principal below; an individual IAM user cannot grant itself above the SCP) + VS Code policy (admin policy overrides user settings, the user cannot escape it) + Active Directory **Enforced** GPO (a higher-OU enforced policy wins over lower). **No security-grade system lets an individual out-rank a shared resource's authority** — here that rule is satisfied by ranking the **user beneath repo** (the bottom of the authority order), not by removing the user from the order. The user keeps authority over its **own** scope and full value-precedence for its own preferences; it simply cannot bind anything above it.
- **F1.4 — replace vs augment the existing D1 text.** **[OWNER-RATIFIED 2026-06-27 — default adopted.]** *Ratified ruling:* a **CLEAN D1 rewrite** (introduced as **D1a**) that names the two orderings explicitly, **not** a silent augmentation of the existing chain. Rationale (audit-corrected): §15 D1's own text says scope "*merges* into effective **policy**" (`design.md:867`), not just into ordinary values — so the existing chain already *implies* the authority axis it never names. Bolting AUTHORITY-RANK onto that chain in-place would **change shipped semantics invisibly** (a reader of the old D1 assumed one ordering governed both policy and values). A clean D1a that states "there are two orderings" makes the change legible instead of hidden. **Compatibility notes the rewrite must carry:** (i) the existing value-precedence chain is **preserved verbatim** as the VALUE-PRECEDENCE ordering — no shipped value-merge behavior changes; (ii) downstream references to "the D1 scope chain" resolve to the VALUE-PRECEDENCE ordering by default (back-compat); (iii) the AUTHORITY-RANK ordering + source-authority-registry are **net-new** substrate concepts, so nothing that exists today silently re-binds. *Alternative to note:* a pure augment (keep D1, append D1a) — lower edit surface, but risks leaving the old "merges into policy" wording reading as if one chain still governs both, which is the invisible-semantics-change the rewrite exists to prevent.
...
*OWNER-RATIFIED 2026-06-27. All 8 forks are resolved (F1.3, the keystone authority-rank fork, ruled as a single total authority order `org > team > repo > user` with the user as the lowest authority rung — on the chain, beneath repo — plus a separate value order `user / repo / runtime`; see F1.3). This proposal is now the canonical decision record ready to fold into §15. The fold into the shipped `config-distribution-model` §15 spec is a separate gated step; §15 and the other family specs were read, not edited, here.*
