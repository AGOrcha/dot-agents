...
- **F1.3 — default authority-rank ordering.** **[OWNER-RATIFIED 2026-06-27 — this was the LAST open fork; the owner refined the rev-2 recommended default (Option B, `org > team > user > repo`) toward Option C: a strict **TWO-TIER** model that takes the user OFF the authority axis entirely.]** *Ratified ruling — TWO-TIER:*
  - **AUTHORITY tier** (the only scopes that may emit **locks / value-locks / override-caps**): **`org > team > repo`**, **deny-overrides**, **higher binds lower**. A higher authority scope's locks and caps are absolute over every lower one. **repo MAY set its own local guardrails** (resource self-constraint — a repo can lock down *its own* surface) **but ranks BELOW team**, so a team lock still binds the repo. 
  - **The USER is NOT in the authority chain.** A user carries **preferences / values only** — **no authority** over a shared resource. A user can never emit a lock or cap that binds anyone (including their own repos against a higher scope). This is the deliberate refinement away from Option B's `… > user > repo`: the user is moved **off the authority axis**, not merely ranked low on it.
  - **VALUE tier** (choose values **within the locks the authority tier leaves surviving**): **`user / repo / runtime`**, **most-local-wins**. This is the Phase-2 value-precedence merge, constrained by the authority pass.
  ...
  - *Grounding (why user-off-authority-axis):* AWS SCP/IAM (an org SCP caps every account/principal below; an individual IAM user cannot grant itself above the SCP) + VS Code policy (admin policy overrides user settings, the user cannot escape it) + Active Directory **Enforced** GPO (a higher-OU enforced policy wins over lower). **No security-grade system grants an individual authority over a shared resource** — the user supplies values, never binds. This two-tier shape matches every analog; Option B's "user in the authority chain" did not.
...
*OWNER-RATIFIED 2026-06-27. All 8 forks are resolved (F1.3, the keystone authority-rank fork, ruled TWO-TIER with the user off the authority axis — see F1.3). This proposal is now the canonical decision record ready to fold into §15. The fold into the shipped `config-distribution-model` §15 spec is a separate gated step; §15 and the other family specs were read, not edited, here.*
