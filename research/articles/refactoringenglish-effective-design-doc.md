...
Michael Lynch (ex-Google, ex-Microsoft) lays out when a design doc is worth writing, how much to invest in it, and what belongs in one. His core sizing heuristic: a decision belongs in the design doc only if the penalty for getting it wrong is high (language choice, storage backend) — trivially reversible choices ("load more" button) should never consume review cycles. The bulk of the piece is a component-by-component catalog (objective, background, goals/non-goals, scenarios, diagrams, SLOs, security, open/resolved issues, alternatives considered) with worked examples from a fictional caching-layer project, RecencyBank.

### Relevance to dot-agents

Directly applicable to the repo's spec/plan artifact model (`workflow/specs/<id>/design.md`): Lynch's "what's the cost of getting it wrong?" test is a crisp criterion for what belongs in a spec versus a plan, and his open-issues/resolved-issues/alternatives-considered sections map onto the spec's "open questions" and "decisions with rationale" requirements. The investment-sizing section (linked anchor) also informs how much design ceremony to require before fanout in the orchestration loop.
...
- What's the authoritative URL?
  - Especially if your organization uses shortlink redirects like `http://go/recency-bank`

**Example Metadata**

- **Author**: Michael Lynch (michael@refactoringenglish.com)
...
**Example Scenario: Share a report via URL**
...
Once you nail down your SLOs, it's time to think about how you'll measure them in production.
...
- **Third-party packages**
  - bbolt: This is a widely used key-value store implementation that implements many of the features we need for RecencyBank.
...
- What is the attack surface of this system?
...
**Example Open Issue: Choosing RAM size for cache**
...
**Example Resolved Issue: Choosing RAM size for cache**
...
The user's link pointed at the anchor `#how-much-should-you-invest-into-your-design-doc`; the full excerpt page was captured (the anchored section is under "How much should you invest into your design doc?" above). This page is an excerpt of a chapter from Lynch's book/site "Refactoring English"; the closing line links onward to a companion section on gathering review feedback which lives on a separate page and was not captured.
