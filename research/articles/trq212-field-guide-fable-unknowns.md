...
**Date**: 2026-07-03 (1:43 PM)
**Method**: Claude-in-Chrome (logged-in x.com session; full X-native article body)
**Word count**: ~1,900 words
**Engagement**: 261 replies, 1.6K reposts, 9.2K likes, 21K bookmarks, 3.6M views

---

### Summary

The map (prompts, skills, context) is not the territory (the codebase and its real constraints); the difference is your unknowns, and Fable is the first model where output quality is bottlenecked by the human's ability to clarify them. Breaks unknowns into the Rumsfeld quadrants (known knowns = the prompt; known unknowns; unknown knowns = obvious-to-you-but-unwritten; unknown unknowns) and gives a technique catalog by phase — pre-implementation: blind-spot pass, brainstorms/prototypes (HTML artifacts), interviews ("one question at a time, prioritize answers that change the architecture"), references (source code as the best reference), implementation plans that lead with the decisions likely to change; during: an implementation-notes.md logging deviations; post: pitch/explainer artifacts and quizzes you must pass before merging. Worked example: editing the Fable launch video entirely with Claude Code. Sibling to his earlier HTML-over-Markdown piece (separate file: trq212-html-over-markdown.md).

### Relevance to dot-agents

Directly applicable to the delegation-bundle and spec discipline: "unknown knowns" are exactly what write_scope constraints and KG readback are supposed to surface before fanout, and the interview pattern maps onto ideation-cycle fork resolution. The implementation-notes.md deviations log is the iter-log/merge-back pattern stated as a per-session habit; the quiz-before-merge is a human-side analogue of the parent verdict gate. Useful vocabulary for the transcript-analysis pipeline: unknowns discovered mid-implementation are what the pipeline should be able to detect in session logs (deviation points, replans).
...
The best agentic coders are good have relatively few unknowns [sic]. Watching someone like Boris or Jarred prompt, it is obvious to me that they know what they want in-detail. They are deeply in-sync with both the codebase and the model behaviors.
...
To do this, you can ask Claude to help you find your unknown unknowns and explain them to you. I like to use the literal words "blindspot pass" and "unknown unknowns". Giving it context on who you are and what you know is usually important for [sic — sentence truncated in original]
...
> "Before wiring anything up, make a single HTML file mocking the new editor toolbar with fake data. I want to react to the layout before you touch the treal app." [sic]
...
Extracted 2026-07-12 via Claude-in-Chrome with a logged-in x.com session. Body verbatim and complete, including the author's typos/truncated sentences (marked [sic]). This is a DIFFERENT article from the same author's earlier "Unreasonable Effectiveness of HTML" (status 2052809885763747935, captured in trq212-html-over-markdown.md).
