...
| `install` | Set up project from `.agentsrc.json` manifest (`--generate` to create or refresh one — additive: scan-derived skill/agent/rule lists are replaced, but declarations like `hooks`/`mcp`/`settings` and `extends` survive; `--force-generate` lets the scan replace those declarations too). A generated manifest declares only PROJECT-owned `sources` — authored `git`/`http`/`oci` roots and path-bearing `local` layers are kept, while the bare `{"type":"local"}` entry naming your own `~/.agents` is never written or resolved as a project root. EXACT by default — prunes managed shared-target links no longer in the resolved set; pass `--inexact` to keep additive behavior, `--strict` to fail on a missing declared resource |
