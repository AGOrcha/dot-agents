---
name: review-delta
description: Review only changes since last commit using impact analysis. Token-efficient delta review with automatic blast-radius detection via da kg (CRG bridge).
argument-hint: "[file or function name]"
---

# Review Delta

Perform a focused, token-efficient code review of changed code and its blast radius using **`da kg`** (code graph + bridge), not raw MCP calls, unless you are driving **`da kg serve`** for compatibility.
