---
name: build-graph
description: Build or update the code-review knowledge graph via da kg (CRG embedded in the KG subsystem). Run this first to initialize, or let hooks keep it updated automatically.
argument-hint: "[full]"
---

# Build Graph

Build or incrementally update the persistent **code-structure graph** for this repository using the **`da kg`** CLI (same store the MCP server uses when you run `kg serve`).
...
2. **Build or update**
   ...
   Use `da kg build --help` / `kg update --help` for flags (`--repo`, `--skip-flows`, etc.).

3. **Verify**

   Run `da kg code-status` again and report files parsed, node/edge counts, languages, and any errors.
...
Editors or automation that still use **stdio MCP** (`da kg serve`) may call `build_or_update_graph_tool` / `list_graph_stats_tool`; behavior should match the CLI paths above. Prefer **CLI** in agent sessions so the same commands work everywhere without JSON-RPC.
