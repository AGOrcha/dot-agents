...
// addTSFn registers a TypeScript function node in both the per-file and per-name maps.
func (f *fakeReader) addTSFn(qn, file string, start, end int) {
...
// simpleFakeReader returns a fakeReader seeded with a single easy TypeScript symbol:
//
//	src/foo/foo.ts → foo.bar (calls foo.helper)
//
// NodeCount=2, EdgeCount=1, Cyclomatic=2 → easy.
func simpleFakeReader() *fakeReader {
	f := newFakeReader()
	f.addTSFn("foo.bar", "src/foo/foo.ts", 10, 30)
	f.addTSFn("foo.helper", "src/foo/foo.ts", 35, 45)
	f.addCall("foo.bar", "foo.helper")
	return f
}
...
//
//	baz.process calls baz.a, baz.b, baz.c
//	NodeCount=4, EdgeCount=3, Cyclomatic=4 → medium (edge_count gives medium).
func mediumFakeReader() *fakeReader {
	f := simpleFakeReader()
	f.addTSFn("baz.process", "src/baz/baz.ts", 1, 60)
	f.addTSFn("baz.a", "src/baz/baz.ts", 70, 80)
	f.addTSFn("baz.b", "src/baz/baz.ts", 85, 95)
	f.addTSFn("baz.c", "src/baz/baz.ts", 100, 110)
	f.addCall("baz.process", "baz.a")
	f.addCall("baz.process", "baz.b")
	f.addCall("baz.process", "baz.c")
	// Add calls among helpers to increase edge count into medium range.
	f.addCall("baz.a", "baz.b")
	f.addCall("baz.b", "baz.c")
	return f
}
...
func TestLanguage(t *testing.T) {
	...
	if g.Language() != eval.LanguageTypeScript {
		t.Errorf("Language() = %q, want %q", g.Language(), eval.LanguageTypeScript)
	}
}
...
func TestGenerate_VerificationCmdsIncludeTsc(t *testing.T) {
	g := mustGenerator(t, simpleFakeReader())
	spec, err := g.Generate(context.Background(), eval.GenerateOptions{})
	if err != nil {
		t.Fatalf("Generate: %v", err)
	}
	if len(spec.Verification.BuildCmd) == 0 || spec.Verification.BuildCmd[0] != tscCmd {
		t.Errorf("BuildCmd = %v, want first element %q", spec.Verification.BuildCmd, tscCmd)
	}
	// Build command must include --noEmit for type-checking without output.
	found := false
	for _, arg := range spec.Verification.BuildCmd {
		if arg == noEmitFlag {
			found = true
		}
	}
	if !found {
		t.Errorf("BuildCmd %v missing %q flag", spec.Verification.BuildCmd, noEmitFlag)
	}
}

func TestGenerate_VerificationTestCmdUsesNode(t *testing.T) {
	g := mustGenerator(t, simpleFakeReader())
	spec, err := g.Generate(context.Background(), eval.GenerateOptions{})
	if err != nil {
		t.Fatalf("Generate: %v", err)
	}
	if len(spec.Verification.TestCmd) == 0 || spec.Verification.TestCmd[0] != nodeCmd {
		t.Errorf("TestCmd = %v, want first element %q", spec.Verification.TestCmd, nodeCmd)
	}
	// Test command must include --test flag.
	found := false
	...
	if !found {
		t.Errorf("TestCmd %v missing %q flag", spec.Verification.TestCmd, testFlag)
	}
}
...
// right file. add-test-coverage must target a *.test.ts file (its prompt tells
// the agent to write tests); the two implementation templates must target the
// non-test seed file. A template whose prompt says "write foo.test.ts" but
// whose spec names "foo.ts" fails here.
func TestGenerate_ArtifactMatchesPromptTarget(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.tid, func(t *testing.T) {
			...
			isTest := strings.HasSuffix(artifact, ".test.ts")
...
func TestGenerate_NeighborhoodError(t *testing.T) {
	f := simpleFakeReader()
	f.getNodeErrs["foo.bar"] = errors.New("db timeout")
	g := mustGenerator(t, f)
	_, err := g.Generate(context.Background(), eval.GenerateOptions{})
	if err == nil {
...
func TestGenerate_ComplexityError(t *testing.T) {
	// NeighborhoodFor (depth=2) calls GetEdgesBySource("foo.bar") once
...
func allDeriveFailReader() *fakeReader {
	f := simpleFakeReader()
	f.inErrs["foo.bar"] = errors.New("target edge failure bar")
	f.inErrs["foo.helper"] = errors.New("target edge failure helper")
	return f
}
...
func TestSolutionArtifactPath(t *testing.T) {
	...
	tests := []struct {
		...
		{TemplateAddTestCoverage, "src/foo/foo.test.ts"},
	}
	for _, tc := range tests {
...
func TestNeighborList_NoNeighbors(t *testing.T) {
	nbhd := kgquery.Neighborhood{
		Root:  graphstore.GraphNode{QualifiedName: "foo.bar"},
		Nodes: []graphstore.GraphNode{{QualifiedName: "foo.bar"}},
	}
...
func TestNeighborList_WithNeighbors(t *testing.T) {
	nbhd := kgquery.Neighborhood{
		...
		Nodes: []graphstore.GraphNode{
			...
			{QualifiedName: "foo.helper"},
		},
	}
	got := neighborList(nbhd)
	if !strings.Contains(got, "foo.helper") {
...
func TestTaskID_Format(t *testing.T) {
	got := taskID(TemplateImplPureFn, "foo.bar")
	if got != "kg-ts-impl-foo-bar" {
		t.Errorf("taskID = %q, want %q", got, "kg-ts-impl-foo-bar")
	}
}
