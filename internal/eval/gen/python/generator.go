// Package pygen is the Python-language task generator for the R4 eval harness.
...
// Template IDs the Python generator supports.
const (
...
// Generator produces Python eval.TaskSpecs from the knowledge graph.
...
func New(querier *kgquery.Querier) (*Generator, error) {
	if querier == nil {
		return nil, fmt.Errorf("pygen: querier is required")
	}
...
// Language returns eval.LanguagePython, fulfilling the eval.Generator contract.
func (g *Generator) Language() eval.Language { return eval.LanguagePython }

// Generate synthesises one TaskSpec for Python. The returned spec passes
// TaskSpec.Validate.
...
func selectTemplateID(opts eval.GenerateOptions) (string, error) {
	...
	default:
		return "", fmt.Errorf("pygen: unknown template %q", opts.TemplateID)
	}
}
...
func findSeed(ctx context.Context, q *kgquery.Querier, want eval.Difficulty) (seedResult, error) {
	seeds, err := q.SeedSymbols(ctx, eval.LanguagePython, seedLimit)
	if err != nil {
		return seedResult{}, fmt.Errorf("seed symbols: %w", err)
	}
	if len(seeds) == 0 {
		return seedResult{}, fmt.Errorf("no Python symbols found in graph")
	}
...
func buildSpec(tid string, r seedResult) *eval.TaskSpec {
	...
	return &eval.TaskSpec{
		...
		Verification: eval.Verification{
			BuildCmd:       []string{pythonCmd, pytestSub, "py_compile", r.seed.FilePath},
			TestCmd:        []string{pythonCmd, pytestSub, pytestPkg, verboseFlag, dir},
...
// add-test-coverage directs the agent to a sibling test_*.py file, so its
...
// dirPath returns the slash-normalised directory containing filePath, for use
// as the pytest discovery target (e.g. "pkg/foo/utils.py" → "pkg/foo").
// Falls back to "." for root-level or empty paths.
...
func taskID(tid, qualifiedName string) string {
	return "kg-py-" + templateShort(tid) + "-" + sanitizeQN(qualifiedName)
}
...
const (
	...
	promptNoTestEdit     = "- Do not modify any existing test_*.py file.\n"
	promptMustSatisfy    = "- The solution must satisfy: python -m pytest -v %s"
)
