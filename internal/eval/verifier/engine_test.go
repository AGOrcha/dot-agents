...
// All other processes ignore this block. It is re-invoked from this package's
// own test binary (os.Args[0]) so it stays toolchain-free and portable.
...
// newEngine returns a BaseVerifier wired to the real process runner, matching
// what every per-language adapter constructs via NewBase.
func newEngine() *BaseVerifier { return NewBase(eval.LanguageGo) }

// ---- NewBase / Language -------------------------------------------------------

func TestNewBase_Language(t *testing.T) {
	b := newEngine()
	if b == nil {
		t.Fatal("NewBase returned nil")
	}
	if got := b.Language(); got != eval.LanguageGo {
		t.Errorf("Language() = %q, want %q", got, eval.LanguageGo)
	}
}

func TestNewBase_UsesRealRunner(t *testing.T) {
	b := newEngine()
	if b.runCmd == nil {
		t.Fatal("NewBase must set runCmd to a non-nil function")
	}
}
...
func TestVerify_NilSpec(t *testing.T) {
	b := newEngine()
	_, err := b.Verify(context.Background(), nil, t.TempDir(), nil)
	if err == nil {
...
func TestVerify_TestPassNoBuildCmd(t *testing.T) {
	b := newEngine()
	b.runCmd = fakeRunCmd("ok\n", "", 0, 5*time.Millisecond, nil)

	spec := minimalSpec()
	res, err := b.Verify(context.Background(), spec, t.TempDir(), nil)
	if err != nil {
...
func TestVerify_TestExecError(t *testing.T) {
	...
	b := newEngine()
	b.runCmd = fakeRunCmd("", "", 0, 0, execErr)
...
func TestVerify_BuildFailShortCircuits(t *testing.T) {
	calls := 0
	b := newEngine()
	b.runCmd = func(_ context.Context, _ string, _ []string, _ []string) (string, string, int, time.Duration, error) {
		calls++
...
func TestVerify_BuildExecError(t *testing.T) {
	...
	b := newEngine()
	b.runCmd = fakeRunCmd("", "", 0, 0, buildErr)
...
func TestVerify_TimeoutExpires(t *testing.T) {
	...
	start := time.Now()
	result, err := b.Verify(context.Background(), spec, t.TempDir(), env)
...
func TestVerify_TimeoutZeroNoDeadline(t *testing.T) {
	...
	b := newEngine()
	b.runCmd = func(ctx context.Context, _ string, _ []string, _ []string) (string, string, int, time.Duration, error) {
	...
	_, err := b.Verify(ctx, spec, t.TempDir(), nil)
...
func TestVerify_DurationAccumulates(t *testing.T) {
	...
	b := newEngine()
	b.runCmd = fakeRunCmd("", "", 0, stepDur, nil)
...
func TestVerify_EmptyWorkdir(t *testing.T) {
	...
	spec := minimalSpec()
	_, err := b.Verify(context.Background(), spec, "", nil)
	var ve *VerifyError
	if !errors.As(err, &ve) {
...
func TestVerify_CommandRunsInWorkdir(t *testing.T) {
	...
	var got string
	b := newEngine()
	b.runCmd = func(_ context.Context, workdir string, _ []string, _ []string) (string, string, int, time.Duration, error) {
		got = workdir
		return "ok\n", "", 0, time.Millisecond, nil
	}
	spec := minimalSpec()
	res, err := b.Verify(context.Background(), spec, want, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !res.Passed {
