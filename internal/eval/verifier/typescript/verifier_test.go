...
// TestNew_Language asserts the TypeScript adapter reports the TypeScript
// language, promoted from the embedded engine.
func TestNew_Language(t *testing.T) {
	v := New()
	if v == nil {
		t.Fatal("New returned nil")
	}
	if got := v.Language(); got != eval.LanguageTypeScript {
		t.Errorf("Language() = %q, want %q", got, eval.LanguageTypeScript)
	}
}
