...
func (ev *evaluator) applyWhere(rows []binding) ([]binding, error) {
	if len(ev.q.Where) == 0 {
		return rows, nil
	}
	var out []binding
	for _, row := range rows {
		keep, err := ev.rowPasses(row)
		if err != nil {
			return nil, err
		}
		if keep {
...
func (ev *evaluator) rowPasses(row binding) (bool, error) {
	for _, pred := range ev.q.Where {
		ok, err := ev.evalPredicate(pred, row)
		if err != nil {
			return false, err
		}
		if ok {
		...
		return false, nil // required source: drop the row
	}
	return true, nil
}
...
func (ev *evaluator) evalPredicate(pred Predicate, row binding) (bool, error) {
	...
	right, err := ev.evalValueExpr(pred.Right, row)
	if err != nil {
		return false, err
	}
	if pred.Op == "IN" {
	...
	return compare(pred.Op, left, right), nil
}

// evalStartsWith evaluates the STARTS_WITH(field, $param) prefix predicate.
func (ev *evaluator) evalStartsWith(pred Predicate, row binding) (bool, error) {
	left, ok := ev.resolveFieldRef(pred.Left, row)
	if !ok {
		return false, nil
	}
